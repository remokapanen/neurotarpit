...
(Tässä kysymyksessä on ehkä haettu jo sekundaaripreventiivisen hoidon aspektia, mutta siitä enemmän c-kohdassa)
...
AVH:n sekundaaripreventio on todella tärkeää varsinkin alkuvaiheissa, koska AVH:n uusiutumisriski on suurimmillaan ensimmäisten päivien ja 
viikkojen aikana, pitkäaikaisuusintariski 6,4%/vuosi (vaihtelee tietysti etiologian mukaan). Kaikille AVH-potilaille tehdään etiologian mukainen arvio sekundaaripreventiivisestä verenkiertoon vaikuttavasta lääkityksestä ja sen aloitusajankohdasta. 
...
P.k. ENMG. Akuuttiin polyradikuliittiin viittaavaa? Hermo-lihasvauriota?
  </div>
</div>
...
Jos vapina ei lievity levodopalla puhuu se Parkinsonin tautia vastaan
...
Hieman nyanssisesti väärin. Vaikka levodopa auttaa monilla vapinaan erittäin hyvin, noin kolmasosa potilaista kärsii vapinasta, joka ei reagoi levodopaan kunnolla, vaikka lääkeannosta nostettaisiin -> levodoparesistentti lepovapina.

Parkinsonin taudin motorisista pääoireista (lepovapina, rigiditeetti, bradykinesia) epävarmin vaste levodopa-hoidolle on juuri vapinalla. Vapinan syntymekanismi on monimutkaisempi ja siihen liittyy dopamiinin lisäksi myös muita aivojen välittäjäainejärjestelmiä (kuten asetyylikoliini ja serotoniini) sekä pikkuaivojen ratoja. 

Tukena levodoparesistensissa vapinassa voidaan käyttää mm. antikolinergista lääkitystä (esim. biperideeni ’Akineton’), mutta huomioitava antikolinergien haitat; ei myöskään sovellu parkinson-dementiassa käytettäväksi. Laiteavusteisina hoitoina tarvittaessa DBS ja Neuro-HIFU, lääkepumpuista ei ole hyötyä, jos vapina on levodoparesistentti. 
...
Ikä, verenpainetauti, hyperkolesterolemia, tupakkatausta, lihavuus ja kuorsaus (todennäköisesti uniapnea, mutta kuorsaus itsessäänkin on itsenäinen riskitekijä AVH:lle).
...
Yläraajoihin painottuva heikkous ja puheen tuoton häiriöt viittaavat vahvasti MCA-suoneen (a. cerebri media). Kyseessä tietysti vielä vasemman MCA:n ongelma, koska potilaalla oikeanpuoleisia pareesioireita. 
...
ensisijaisesti pään TT:n ja kaulavaltimokuvantamiset (TT-angio). Päivystystutkimuksiin kuuluu lisäksi EKG (telemetria päivystyksessä olon ajan), verenpaineseuranta ja thx-rtg. Rutiinilaboratoriokokeet kuuluvat myös selvittelyihin (perusverenkuva ja trombosyytit, CRP, kalium, natrium, kreatiniini, glukoosi, P-TT tai INR (varfariinihoito), P-APTT). Oireiden uusiessa on huomioitava myös liuotushoidon mahdollisuus, mikäli hoidon aikaikkuna ei ole umpeutunut. 
...
Iskeemisissä AVH-tapauksissa usein tarvitaan sekundaaripreventiivisesti antitromboottista tai antikoagulatiivista lääkitystä. Jos ei löydetä eteisvärinää (hoitona AK-lääkitys, yleensä NOAC), niin kyseessä on todennäköisesti ateroskleroottinen (tai pienten suonten) ongelma -> suuren riskin TIA:ssa (ABCD2 ≥ 4) ASA 100 mg × 1 + klopidogreeli 75 mg × 1 kolmen viikon ajan (jos ei ole aiemmin ollut käytössä, niin hoito aloitetaan latausannoksella 250mg ASA tai klopidogreeli 300mg), jonka jälkeen yleensä klopidogreeli 75 mg × 1. 

<li>Pienen riskin TIA:ssa klopidogreeli 75 mg × 1, ASA:n ja dipyridamolin yhdistelmä 25/200 mg × 2 tai joskus pelkästään ASA 100 mg × 1</li>
...
<li>Päihteet (esim. alkoholi), korkeakuumeinen infektio, keskushermostoinfektio, hypoglykemia, elektrolyyttihäiriöt, äärimmäiset rasitustilat (valvominen, maratonin juokseminen, voimamieskisat...) ja jotkut kohtauskynnystä madaltavat lääkkeet (trisykliset masennuslääkkeet, bupropioni, useat psykoosilääkkeet, tramadoli)</li>
...
Pään TT poissulkututkimukseksi (käytännössä aina ensikouristajille!)
...
Ajokielto tulee jo koko epilepsiaselvityksien ajaksi. Yksittäisen epileptisen kohtauksen jälkeen ajokielto on R1 3kk ja R2 5v jo heti päivystyksessä määrättynä (vaikka kohtauksen ajateltaisiinkin olevan altisteinen, kuten alkoholikouristus). Potilaalle siis määrätään sairaslomaa vaarallisista tehtävistä kuten rekan ajamisesta. 

Jos diagnosoidaan epilepsia tai todetaan 2 epileptistä kohtausta alle 3v. sisään, tulee kielto olemaan R1 vähintään 1v (ajoluvan palautukseen vaaditaan 1v kohtauksettomuus)ja R2 käytännössä pysyvä (10v ilman lääkitystä kohtaukseton -> mahdollista saada takaisin).  
  </div>
</div>
...
Todennäköinen syy on selkäytimen kompressio, esimerkiksi **maligniteetin,** välilevytyrän, absessin tai trauman aiheuttamana. Mahdollisesti kyseessä voi olla transversaalimyeliitti.

Rintarangan alue on selkärangasta se segmentti, jossa on suurin metastaasien esiintyvyys (rintaranka (n. 60-80%) > lanneranka (n.  15-30%) > kaularanka (<10%)). Yleisimmät lähteet rintarankametastaaseille on rinta- ja keuhkosyövät (lannerangassa taas eniten mm. GI- ja eturauhassyövät). 
  </div>
</div>
...
Selkärangan MRI päivystyksellisesti; erityisesti siis rintarangan (ja mahdollisesti kaularangankin, mutta rintarangan ongelmasta todennäköisesti kyse, kun navan alueella tuntoraja (Th10-dermatomi on navan alue)). Lannerangan alueella ei enää ole selkäydintä (loppuu yleensä viimeistään L1/L2-tasolle), joten sen lannerangan alueen leesiot eivät aiheuta ylämotoneuronilöydöksiä. 
  </div>
</div>
...
GBS:n hoito toteutetaan päivystyksellisesti sairaalassa, jossa on hengitystuen mahdollisuus (voi edetä arvaamattoman nopeasti hengityslamaan ja erotusdiagnostiikkaan myös tarvitaan ESH:n tutkimuksia (likvor, MRI ja ENMG)). Hoitona on yleensä IVIG 0,4 g/kg /pv 5 päivänä; plasmafereesi nykyään harvemmin käytetty vaihtoehto, koska laskimoon annosteltava immunoglobuliini on paremmin siedetty ja se on helpommmin toteutettavissa. Vaikeissa tapauksissa plasmafereesiä vielä kuitenkin käytetään usein. 

Periaatteessa GBS paranee itsestäänkin, mutta ilman hoitoa riski pysyvistä vaurioista ja kuolemasta on suurempi. Joskus, jos GBS on erittäin lievä, saatetaan päätyä pelkkään seurantaan sairaalassa.
  </div>
</div>
...
Tietysti myös oireiden äkillisyys ja onko kohtauksellista (kesto tärkeää selvittää)/jatkuvaa, onko traumaa taustalla, onko muita liitännäisoireita (neurologiset, päänsärky, kuulonalenema, infektiot...), mitä lääkkeitä potilas käyttää, muut sairaudet ja sukuanamneesi. 
...
<li>Yläraajojen lihasheikkous, jonka yhteydessä atrofiaa, faskikulaatioita -> alamotoneuronin vaurio. Kuitenkin samalla vilkastuneet yläraajarefleksit sekä positiivinen Hoffmannin testi -> ylämotoneuronin vaurio.</li>
  <ul>
    <li>Hoffmannin testi = Yläraajojen ekvivalentti Babinskille = Ylempää motoneuronia testaava yläraajan testi, jossa potilaan keskisormen kynttä napautetaan nopeasti alaspäin. Positiivisessa tuloksessa potilas koukistaa ja abduktoi peukalonsa. Samalla voi tulla etusormen fleksio.</li>
  </ul>
...
<li>Aaltojen polyfasiaa</li>
<li>Denervoituneissa lihaksissa fibrillaatioita ja suuriamplitudisia motorisen yksikön aktiopotentiaaleja (large amplitude MUAPs)</li>
...
<li>Kuvaus oireista; miksi otettu pään kuva. Halvaukset, aivohermojen toiminta, kognitio, yleisesti neurologiset ongelmat? Pahoinvointi, huimaus, päänsärky, tajunnan taso? Kouristelua? Miten alkoi? Pään vammaa taustalla?</li>
  <ul>
    <li>Selvä SAV-epäily (räjähdysmäisesti alkanut, elämän kovin päänsärky, usein mukana oksentelua ja niskajäykkyyttä), vaikka pään TT olisikin normaali (tosin <6h oireiden alusta otettu TT on lähes 100% herkkyydeltään SAV:n poissulun suhteen), on liuotushoidon vasta-aihe -> usein tarkistetaan lannepiston kautta, onko verta likvorissa.</li>
  </ul>
...
<li>Esitiedoista perussairaudet, aikaisemmat toimenpiteet/verenvuodot ja lääkitys</li>
  <ul>
...
<li>Labroissa nopeasti pika-gluk (yleensä ainoa labra, joka tarvitaan ennen liuotusta (pika-INR, jos varfariinin käyttäjä), jotta voidaan poissulkea merkittävä hypoglykemia); otetaan myös AVH-paketissa PVKT, na, k, krea, CRP, Gluk, TnT, CK, INR, APTT</li>

---

Lisäksi tietysti neurologinen status (NIHSS-arvio): 

<li>Puheen tuotto ja ymmärrys</li>
<li>Neglect</li>
<li>Silmät (osoittaako samaan suuntaan, devioiko, mustuaisissa eroja, reagoiko valoon, näkekentät...)</li>
...
Yleensä etiologia on tromboottinen tai embolinen. Iskeemisen infarktin lisäksi voi olla myös hemorraginen infarkti, mutta yleensä kun puhutaan aivoinfarktista, niin puhutaan iskeemisestä infarktista. Hemorragiset infarktit poissuljetaan akuuttivaiheessa pään natiivi-TT-kuvalla.
...
<li>Paradoksaalinen embolisaatio: SLT embolisoituu ja potilaalla avoin kammioväliseinäaukko -> SLT suhteen status ja alaraajojen UÄ (SLT:n etsintä) sekä sydänultra (ASD:n etsintä)</li>
...
AVH:n sekundaaripreventio on todella tärkeää varsinkin alkuvaiheissa, koska AVH:n uusiutusmisriski on suurimmillaan ensimmäisten päivien ja 
viikkojen aikana, pitkäaikaisuusintariski 6,4%/vuosi (vaihtelee tietysti etiologian mukaan). Kaikille AVH-potilaille tehdään etiologian mukainen arvio sekundaaripreventiivisestä verenkiertoon vaikuttavasta lääkityksestä ja sen aloitusajankohdasta. 

<li>Jos potilaalla on eteisvärinä, niin AK-hoito on ensisijainen sekundaaripreventiivinen hoito. Nykyään ensisijaisesti käytetään NOAC-lääkitystä (varfariini jos läppäperäinen eteisvärinä). Eteisvärinää pitää aktiivisesti etsiä AVH-potilailta. </li>
<li>Jos potilaalla on suurten suonten ateroskleroosi (pääasiassa kaulavaltimot) ja AVH:n puolella merkittävä kaulavaltimostenoosi (>50%), niin ensisijainen hoito on endarterektomia 2vk sisällä siihen soveltuvilla ja jatkoon klopidogreeli tai ASA+pyridamoli. Jos ahtauman aste on alle 50 %, niin ensisijainen hoito on pelkkä lääkehoito ilman kirurgiaa.</li>
<li>Pienten suonten taudissa tehdään vuotoriskin arvio ja jos se on pieni, niin ensisijaisesti aloitetaan klopidogreeli tai ASA+pyridamoli. Jos vuotoriski on iso, niin ASA yleensä.</li>
<li>Kaulavaltimodissekaatiossa infarktitilanteessa voidaan akuuttivaihe hoitaa liuotuksella kriteerien muutoin täyttyessä. Lääkehoidon tavoite liuotushoidon jälkeen – ja silloin kun potilas ei ole sitä saanut – on ehkäistä iskemian uusiutuminen ja lääkkeinä käytetään antikoagulaatiohoitoa tai asetyylisalisyylihappoa (ASA) tai DAPT-hoitoa, joista AK-hoito (NOAC) on Suomessa yleisin. Hoitoa jatketaan yleensä kuuden kuukauden ajan, minkä jälkeen, jos on jäänyt merkittävä stenoosi (arvioidaan kontrolli-CTA:lla/MRA:lla), siirrytään asetyylisalisyylihappoon. </li>
...
Tensiopäänsäryssä ei yleensä esiinny migreenityyppisiä liitännäisoireita. Joskus harvoin näitä todetaan, mutta silloin vain yksi lievä oire kerrallaan; esim. joskus kävellessä huimausta tai lievää pahoinvointia. Varsinaista oksentelua ei sinänsä tulisi tensiopäänsäryssä esiintyä. 
...
Trigeminusneuralgian akuuttihoidossa auttaa 100% hapen hengittäminen
...
<li>Huomio kohdistuu äkilliseen yksipuoleiseen kivuttomaan alaraajapareesiin, jonka yhteydessä potilaan kognitio mahdollisesti alentunut</li>
...
<li>Onko räjähdysmäisesti alkanutta, elämän kovinta päänsärkyä? Onko oksentelua tai niskajäykkyyttä?</li>
  <ul>
    <li>Selvä SAV-epäily, vaikka pään TT olisikin normaali, on liuotushoidon vasta-aihe (tosin <6h oireiden alusta otettu TT on lähes 100% herkkyydeltään SAV:n poissulun suhteen) -> usein tarkistetaan lannepiston kautta, onko verta likvorissa.</li>
  </ul>
...
<li>Tästä huolimatta potilaalla on silti ongelmia tilanvaihteluiden kanssa -> **on aika erikoishoidoille.** Parkinsonin taudin edenneessä vaiheessa oirekuva alkaa usein vaatimaan laiteavusteista hoitoa. Käytettävissä on muutama eri hoitomuotoa kuten lääkeinfuusiot/-pumput, syväaivostimulaatio (DBS) tai neuro-HIFU</li>
  <ul>
    <li>Lääkkeellisistä hoidoista on tarjolla jatkuva levodopa-karbidopa- tai levodopa-karbidopa-entakaponi-infuusiopumppu suoleen, jatkuva foslevodopa/foskarbidopa-infuusiopumppu s.c. ja apomorfiini-infuusiopumppu s.c.</li>
