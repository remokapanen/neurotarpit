...
<li>Vaikutus toimintakykyyn? Tämä on tärkeää kysyä aina lihasheikkouspotilailta. Esim. Mihin oirekuva vaikuttaa? Työkyky? Urheilusuoritukset? Kävelymatka? Portaiden nousu? Puhuminen, nieleminen? Kierrekorkkien avaaminen? Hiusten peseminen? Apuvälineiden tarve?</li>
...
Potilas A: Oirekuva viittaa primaariin päänsärkyyn, mutta mukana on joitakin hälyttäviä piirteitä, kuten "elämän pahin päänsärky" ja ehkäisytabletit (tromboosiriski). Tässä on mahdollinen epäily aivojen sinustromboosista siis (ilmenee yleensä päänsärkynä ja intrakraniellin hypertension oireina (esim. pahoinvointi/oksentelu, papillödeema)). Sinustromboosin oireet määräytyvät tukkeutuneen sinuksen mukaan ja oireisto voi kehittyä akuutisti, mutta useimmiten päivien-viikkojen kuluessa progressiivisesti. Oireet ja statuslöydökset voivat olla hyvin lieviä ja päänsärky voi olla ainoakin oire. Hänenkin tapauksessa voisi siis olla järkevää toteuttaa kuvantamistutkimuksia. Tehdään kuitenkin tarkempi status (arvioidaan fokaaliset neurologiset oireet, silmänpohjatutkimus ja testataan, paheneeko päänsärky kumartuessa; päänsärkypotilailta kannattaisi myös aina tarkistaa vähintäänkin verenpaine). Konsultoidaan neurologia ja keskustellaan kuvantamisen tarpeesta. Diagnoosi varmistuu aivojen magneettikuvauksella ja siihen yhdistetyllä magneettiangiografialla (laskimosarjat).

Keskustellaan neurologin kanssa myös lannepiston tarpeellisuudesta (kuitenkin niskajäykkyyttä, päänsärkyä ja valonarkuutta; tosin kuulostavat anamneesin perusteella enemmänkin migreeniin ja lihasperäiseen paremmin sopivaksi). Hänellä ei kuitenkaan ole kuumetta eikä tajunnantason alentumista (muutosta tajunnassa ei tosin muutenkaan olisi virusmeningiitissä), joten CNS-infektio on suhteellisen epätodennäköinen. 

Periaatteessa oireiden taustalla voisi olla myös mm. kaulavaltimodissekaatio.
  </div>
</div>
...
Potilas B: SAV:n varmistuessa välitön neurokirurginen konsultaatio (voi ottaa TT-angiografian ennen konsultaatiotakin). SAV-potilas kuuluu akuutisti tehohoitoon tai tehovalvontaan, ei vuodeosastolle. Välittömän uusintavuodon ehkäisyssä mahdollisesti traneksaamihappo; myös verenpaineiden kontrolloiminen tärkeää. Viivästyneitä iskeemisiä aivovaurioita yritetään yleensä ehkäistä lääkehoidolla (aivovaltimospasmeja estetään nimodipiinilla). Mitataan myös mm. kuume, sokerit, elektrolyytit ja muut ongelmat, kuten verenpaineongelmat, pahoinvointi ja epilepsiakohtaukset ja hoidetaan ne. 

Jos kuvantaminen jää negatiiviseksi, niin voidaan mahdollisesti miettiä lannepistoa, josta voidaan nähdä visuaalisesti ksantokromiaa (keltaisuutta) tai analyysissä/spektrissä punasoluja/niiden hajoamistuotteita, jos on SAV. On kuitenkin hyvä tiedostaa, että TT-kuva on n. 99% herkkyydeltään SAV:n poissulussa, jos se otetaan <6h oireiden alusta.

Jos vuoto todetaan, tehdään yleensä heti perään verisuonten kuvantaminen (TT-angiografia on primääritutkimus, tarvittaessa vielä lisäksi katetriangiografia (DSA)) mahdollisen aneurysman paikallistamiseksi (SAV:n taustalla usein aneurysman rupturoituminen). Vuotanut aneurysma tulee eristää viimeistään seuraavana päivänä uusintavuodon estämiseksi. Aneurysman anatomia ja hoitotiimin kokemus ratkaisevat valinnan kirurgisen ja suonensisäisen hoidon välillä: 
...
Parkinsonin taudin diagnoosi on kliininen (ei spesifisiä biomarkkereita) ja perustuu parkinsonismin diagnoosiin sekä sekundaaristen syiden poissulkemiseen (esim. PVK+T, TSH, Ca-ion, ALAT, Na, K, Krea, B12 ja erityisesti käytössä olevan lääkityksen (sekundaarista parkinsonismia voi aiheuttaa esim, neurolepti, metoklopramidi, litium, valproaatti) selvittäminen; joskus myös rakenteelliset syyt, kuten kasvaimet ja vammat). 
...
Aivoverenkiertohäiriöiden oireiden tunnistaminen opetetaan siviileillekkin BE FAST-muistisäännön (tai ehkä useammin pelkkä FAST) kautta. 
...
<li>Onko räjähdysmäisesti alkanutta, elämän kovinta päänsärkyä? Onko oksentelua tai niskajäykkyyttä?</li>
  <ul>
    <li>Selvä SAV-epäily, vaikka pään TT olisikin normaali (ainakin >6h oireista, koska <6h sisällä TT:n herkkyys SAV:n havaitsemiseksi on lähes 100%), on liuotushoidon vasta-aihe -> voidaan tarkistaa lannepiston kautta, onko verta likvorissa.</li>
  </ul>
...
<li>Mikä verensokeri?</li>
  <ul>
    <li>Gluk <2.8 on vasta-aihe liuotukselle (korjataan ensin >4)</li>
  </ul>
...
<li>Onko käytössä antikoagulaatiohoito</li>
  <ul>
    ...
    <li>Jos käytössä on varfariini tai on muutoin syytä epäillä hyytymisjärjestelmän häiriötä, niin mitataan INR. INR >1.7 on vasta-aihe liuotukselle.</li>
      <ul>
        <li>Periaatteessa monissa paikoissa liuotuslabrat ovat yhdessä paketissa ja INR mitataan siten kaikilta. Sitä ei kuitenkaan enää oteta pika-INR-tutkimuksena, jos ei ole ym. syytä.</li>
      </ul>
  </ul>
<li>Potilaan toimintakyky ja omatoimisuus (mRS) ja muut sairaudet (esim. mikä voisi olla taustalla (vaikka eteisvärinää?))</li>
<li>Onko taustalla aiempi spontaani aivoverenvuoto (koskaan), GI-kanavan vuoto (3vk sisällä) tai neurokirurginen leikkaus/vaikea aivovamma/laaja aivoinfarkti (3kk sisällä).</li>
...
Aivoinfarktin akuuttihoito voidaan nykyään toteuttaa tyypillisen tPA-liuotushoidon sijaan DAPT-hoidolla, jos potilaalla on NIHSS < 5 pistettä ja oireet eivät ole invalidisoivia. Aluksi loudaus ASA 250mg ja klopidogreeli 300mg. Jatkohoito ASA 100mg 1x1 ja klopidogreeli 75mg 1x1 3 vko ajan, jonka jälkeen jatko ensisijaisesti klopidogreeli 75mg 1x1 pysyvästi (tietysti jos etiologia on eteisvärinä, niin sekundaaripreventiona AK-hoito). 
...
Hoidon ja kotiutumisen jälkeen: 

Mikäli tiedossa ei ole muistisairautta, ovat muistitutkimukset aiheellisia 3-6 kuukauden kuluttua. Seurannassa jopa kolmanneksella deliriumpotilaista on todettu aiemmin 
diagnosoimaton muistihäiriö.

Asetetaan potilaalle ajokielto, kunnes on toipunut deliriumista ja ajoterveyttä on arvioitu uudelleen. Kirjataan tilapäinen ajokielto potilasasiakirjoihin ja tulostetaan se potilaalle mukaan -> PTH:ssa ajokunnon arviointi uudestaan myöhemmin. 
  </div>
</div>
...
Keskeisintä on potilaan rauhoittaminen (vakuuttaminen oireen hyvänlaatuisuudesta), koska hän pelkää aivokasvainta. Neurologinen status oli normaali ja oireisto (pantamainen, symmetrinen, ei pahene rasituksessa) on tyypillinen jännityspäänsärylle. Pelkkä päänsärky on äärimmäisen harvoin ainoa aivokasvaimen oire. 
...
Sairaalaan lähettämiselle ei ole tarvetta. Jos ilmenee punaisia lippuja päänsäryn suhteen, niin silloin uusi arvio. 
  </div>
</div>
...
Migreenin hoidossa yleensä pidetään päänsärkypäiväkirjaa, joka auttaa tilanteen kartoituksessa ja hoitovasteen seurannassa sekä altistavien tekijöiden tunnistamisessa. Estohoidon aloittamiselle ei ole ehdotonta kohtausmäärärajaa.  Kun potilas kokee migreenikohtauksista sen verran haittaa, että hän on itse halukas käyttämään säännöllistä lääkitystä niiden vähentämiseksi, niin voi kokeilla. Aloitetaan lähtökohtaisesti pth:ssa, jossa tarvittaessa kokeillaan ainakin 2-3 eri estolääkettä ennen lähettämistä neurologin arvioon. 

Migreenin estohoitoon on tähän saakka käytetty PTH:ssa ensisijaisesti angiotensiinireseptorin salpaajia (pääasiassa kandesartaani), trisyklisiä masennuslääkkeitä (esim. amitriptyliini tai nortriptyliini), beetasalpaajia (esim. propranololi) sekä harvemmin eräitä epilepsialääkkeitä (esim. topiramaatti tai harvemmin valproaatti). Jos ensimmäinen migreeniä ehkäisevä lääke on tehoton tai huonosti siedetty, suositellaan lääkevaihtoa. Yhdistelmälääkityskin on mahdollista.

Uutena lääkeryhmänä ovat ESH:n käyttöön tulleet CGRP-vasta-aineet (monoklonaalisia biologisia lääkkeitä, kuten fremanetsumabi tai erenumabi) ja gepantit (pienimolekyyliset CGRP-reseptoriantagonistit, kuten rimegepantti tai atogepantti). Kela-korvattavuus näille myönnetään, kun esiintyy keskimäärin vähintään 8 migreenipäivää kuukaudessa ja on kokeiltu vähintään kahta migreenin estohoitolääkettä (esim. kandesartaani ja amitriptyliini), eikä niillä ole saatu aikaan riittävää vastetta, ne ovat vasta-aiheisia tai ne eivät ole siedettyjä. Näiden kriteerien täyttymistä ennen ei siis yleensä kannata lähettää potilasta ESH:hon estolääkityksen tehostamista varten. Jatkokorvausoikeudet voi hyväksyttää hoitavan lääkärin toimesta, mutta ensimmäinen määräys tarvitsee yleensä neurologin lausunnon. Kroonisen migreenin hoitoon käytetään myös joskus onabotuliinitoksiini s.c. Raskauden aikana vaihtoehtoina lähinnä metoprololi ja propranololi.
...
Myös statuksessa on usein osoitettavissa epäjohdonmukaisuuksia, kuten esimerkiksi tällä potilaalla vapinan loppuminen potilaan keskittyessä muuhun tehtävään (kuukausien luetteluun) -> distraktio eli harhautettavuuslöydös. Oireiden yleinen korostuminen vastaanottotilanteessa on myös yleistä. Kohtauksen aikainen raajojen asynkroninen liikehdintä ja välitön toipuminen myös puhuvat toiminnallisen kohtauksen puolesta. 
...
<li>muistitestit (CERAD vanhuksille ensisijainen, MMSE yksinään seurannassa (ei riitä seulontatestiksi); MoCA myös käytössä paljon varsinkin työikäisillä, mutta työikäisillä tutkimuksissa pääsääntöisesti tarvitaan neuropsykologisia selvittelyitä tarkemminkin); yleensä muistihoitaja tekee -> usein potilas lähetetään esim. Varhan alueella Muistikeskukseen selvittelyihin</li>
...
Likvoranalyysissa voidaan nähdä fosforyloituneen tau-proteiinin (myös yleensä kokonais-taun) suurentuminen ja/tai beeta-amyloidi 42:n pieneneminen. Likvortulosten perusteella voidaan Erlangen scoren (perustuu Aβ42-, Aβ40-, kokonais-tau- ja fosforyloitunut tau-mittauksiin) perusteella arvioida Alzheimerin taudin todennäköisyyttä.
...
AK-hoito tai antitrombootit eivät sinänsä vaikuta itse verenkiertoon, vaan veren hyytymiseen ja verihiutaleiden aggregoitumiseen. Tässä tapauksessa tentissä ei kuitenkaan yritetty mitään kompakysymystä, vaan nämä lääkkeet miellettiin "verenkiertoon vaikuttaviksi" -> vastaus on oikein. 
...
<li>Toimintojen varmentuminen ja sujuvoituminen: 1-2 v (aktiivista kuntoutusta jatketaan yleensä niin kauan kuin oleellista toipumista tapahtuu; toimintakykyä voidaan yleensä parantaa avokuntoutuksella kotiutumisen jälkeen ainakin vuoden ajan)</li>
...
### Huimauksen selvittely

Toimit keskussairaalan päivystäjänä. Illalla ensihoito tuo potilaan arvioon voimakkaan huimauksen, pahoinvoinnin ja tasapainovaikeuksien vuoksi. Potilas on 49-vuotias mies, jolla on jo vuosia ollut kohonneen verenpaineen hoitoon käytössä Losartan 50 mg x 1. Ei tiedossa muita pitkäaikaissairauksia. Oireisto on alkanut noin viisi tuntia sitten. Tuolloin potilas istui sohvalla paikallaan kännykkää selailemassa, kun hänelle alkoi varsin nopeasti, oman arvionsa mukaan minuutissa tai parissa, kehittyä voimakasta kiertohuimausta ja pahoinvointia. Ympäristöstä tukea ottaen hän on kaatumatta päässyt vessaan ja käynyt oksentamassa muutaman kerran. Muilta osin hän on lähinnä maannut paikallaan odottaen oireiston helpottamista, mutta sen jatkuessa samanlaisena päätynyt lopulta soittamaan hätänumeroon. Tapaat edelleen huimaavan ja pahoinvoivan potilaan makaamassa paikallaan oikealla kyljellään ja huomaat heti ensimmäisenä hänellä olevan jatkuva spontaani oikealle lyövä horisontaalistorsionaalinen nystagmus. 


- a. Mitkä ovat kaksi keskeisintä diagnoosivaihtoehtoa tässä tilanteessa? (2 p) 
- b. Mitkä ovat keskeisimmät kliiniset statustutkimukset, joilla jatkat tilanteen selvittelyä ja miten tulkitset niiden löydöksiä? (4 p) 
...
Jatkuva huimaus ja spontaani nystagmus -> kyseessä on akuutti vestibulaarisyndrooma (AVS), joka sisältää keskeisinä erotusdiagnostisina vaihtoehtoina sentraalisen syyn (pääasiassa akuutissa tilanteessa takakierron aivoverenkiertohäiriön) ja vestibulaarineuriitin. Molemmat niistä voivat aiheuttaa nopeasti kehittyvää ja jatkuvaa huimausta sekä jatkuvaa ja spontaania nystagmusta sekä pahoinvointia ja oksentelua, kuten tässäkin tapauksessa.
  </div>
</div>
...
Statuksessa on tärkeää selvitellä heti, voisiko ilmentyä jotain selkeitä sentraaliseen syyhyn viittaavia löydöksiä, kuten fokaalisia neurologisia puutoksia (esim. pareesit), merkittävää päänsärkyä/kaulasärkyä (aivoverenvuoto tai kaulavaltimodissekaatio), kaatavia kävelyongelmia, ns. takakierron deadly D's eli dysartria, dysfonia, dysfagia, dysdiadokokinesia, diplopia tai dyskonjugaatio. Jos ei ole selkeää viitettä näiden perusteella sentraalisesta syystä, niin tulee suorittaa HINTS plus-statustutkimuspatteristo, jonka perusteella voidaan yleensä tarkasti poissulkea sentraalinen syy. 

HINTS plus koostuu sen nimensä mukaisesti HI-testistä eli head impulse -testistä (pään nykäisytesti), jossa normaali löydös (eli ei havaita silmien korjaavaa sakkadiliikettä) on AVS-potilaalla huolestuttava merkki ja viittaa mahdolliseen sentraaliseen etiologiaan. N-osa viittaa nystagmukseen, eli arvioidaan, onko nystagmus vertikaalista (viittaisi sentraaliseen) vai onko se bidirektionaalista (eli nystagmuksen suunta vaihtuu ääriasennoissa; viittaisi sentraaliseen; vestibulaarineuriitissa taas nystagmus on unidirektionaalista horisontaalista kohti tervettä puolta). Sitten TS-osa viittaa test of (vertical) skew-testiin eli piilotetaan potilaan toinen silmä käden alle ja arvioidaan se taas paljastaessa, nähdäänkö silmän vertikaalista virheasentoa ja paljastamista seuraavaa vertikaalista korjausliikettä (viittaisi sentraaliseen syyhyn; pelkästään horisontaalinen ei ole suoraan huolestuttava). Plus-osio tarkoittaa kuulon testaamista bedside (onko siis uutta kuulonalenemaa -> viittaisi sentraaliseen syyhyn). Kuulon testaaminen (Esim. sormien hankaaminen korvan lähellä -> kuuleeko normaaliin tapaan ja symmterisesti) on lisätty klassiseen patteristoon sen takia, että saataisiin paremmin kiinni AICA-infarkteja. 

Jos HINTS plus on negatiivinen sentraalisen syyn puolesta (eli HI havaitaan sakkadi, nystagmus ei ole bidirektionaalista/vertikaalista, ei havaita vertikaalista virheasentoa/korjausta TS:ssä ja kuulossa ei ole muutosta), niin on sentraalinen syy luotettavasti poissuljettu AVS-potilaalta. 

![](images/vertigoalgo.png)
  </div>
</div>

### Aivoiskemia vai epileptinen kohtaus? 

- a. Anna molemmille määritelmä. (1 p)
- b. Kuvaile potilaan oirekuva, jossa aivoiskemian ja epileptisen kohtauksen erottaminen voi olla haastavaa. Perustele miksi, ja kerro mitä hyödynnät tilanteessa erotusdiagnostiikan apuna sairaalapäivystyksessä. (3 p)
...
Aivoiskemia tarkoittaa aivokudoksen hapen puutetta alueen heikosta verenkierrosta johtuen. Epileptinen kohtaus taas tarkoittaa poikkeuksellisen aivojen sähköisen purkaustoiminnan aiheuttamaa kohtausta, joka voi ilmentyä monella eri tavalla, kuten vaikka kouristeluina.
  </div>
</div>
...
Varsinkin ensikouristajilla voi aivoiskemian ja epileptisen kohtauksen erottaminen olla vaikeaa, koska ensikouristajalla ei ole tiedossa hänelle stereotyyppisiä kohtauksia, joiden avulla voitaisiin päätellä, että kyseessä on taas hänelle tyypillinen epileptinen kohtaus. Esimerkiksi vaikka epileptisen kohtauksen sairastaneella potilaalla voi olla kouristuksen jälkeinen ns. Toddin pareesi, jossa kouristanut raaja on heikko kouristuksen jäljiltä, kuten voisi hyvinkin tapahtua aivoiskemiankin aiheuttamana. 

Erotusdiagnostiikassa on todella tärkeää kerätä hyvä anamneesi ja kohtauskuvaus (myös silminnäkijää hyödyntäen). Millaisia oireita oli ennen kohtausta (epileptisissä kohtauksissa voi olla mahdollisesti joitain auraoireita, kuten vaikka outoja aistimuksia tai deja-vu), kohtauksen aikana ja millaisia oireita sen jälkeen (post-iktaalinen tila viittaa vahvasti epileptiseen kohtaukseen); tämä auttaa myös synkopeekohtauksen erottelussa. Kuinka nopeasti oireet tulivat (epilepsiassa voi olla ns. marching-ilmiötä, kun taas AVH:ssa oireet yleensä heti vahvat).

Tulee selvittää mahdolliset altistavat tekijät (epileptiselle kohtaukselle esimerkiksi runsas alkoholinkäyttö sekä vieroitus siitä, muut huumeet, kouristuskynnystä alentavat lääkkeet (Esim. bupropioni tai klotsapiini) merkittävät rasitustilat ja pitkä valvominen, sukuhistoria, infektio-oireisto, toiminnallisissa kohtauksissa psykiatrinen ja sosiaalinen tausta; aivoiskemiassa yleiset verisuoniriskitekijät, kuten alkoholi, tupakointi, ylipaino, kuorsaus, uniapnea, hyperlipidemia, diabetes, korkea verenpaine yms. sekä myös tietysti embolisia ongelmia ajatellen esim. eteisvärinä). 

Neurologinen status ja yleisstatus (vamman merkit, esim. puremajäljet lateraalisesti kielessä viittaa epileptiseen kohtaukseen; samoin virtsainkontinenssin merkit) on myös tärkeä tietysti. Jos potilaalla on katsedeviaatiota, niin strokessa deviaatio on yleensä hemipareesin toiselle puolelle (katsoo vaurioon), kun taas epileptisen kohtauksen aikana se voi olla hemipareesin puolelle. 

Epäselvissä tilanteissa ja muutenkin aina ensikouristajilta otetaan aina pään TT päivystyksessä (samoin tietysti, jos AVH-epäily). Se auttaa myös aivoiskemian ja aivoverenvuotojen tutkimisessa (iskeemisiä muutoksia ei akuuttivaiheessa aina tule heti esille eikä tulekaan, jos kyseessä oli TIA). Tarvittaessa sitten aivoiskemiaa epäiltäessä myös yleensä TT-angiografia. EKG ja telemetriaseuranta kuuluu myös aina päivystyksellisiin tutkimuksiin. Samoin labrat, kuten gluk (erityisen tärkeä ja otetaankin yleensä nopeasti), pvk, na, k, ca, krea, astrup, ck, alat, alkoholia epäiltäessä puhallus, peth, gt, alat, huumeita epäiltäessä u-huum. EEG voidaan ottaa myös päivystyksessä, jos potilas jatkaa kouristelua tai kouristaa uudestaan, mutta yleensä sitä tarvitaan vasta epileptisten kohtausten jatkoselvittelyssä polikliinisesti (yleensä siellä myös MRI). 


  </div>
</div>

### Kaksoiskuvia ja puheen epäselvyyttä

24v. nainen, jolla ei todettuja sairauksia, hakeutuu terveyskeskuksessa kiireettömälle vastaanotollesi. Hän kertoo, että edeltävän kahden kuukauden ajan on huomannut kaksoiskuvia ja puheen epäselvyyttä, joka esiintyvät iltaa kohdin. Aamuisin oireita ei ole esiinny. Oirekuva esiintyy lähes päivittäin. Potilas on myös huomannut, että aiemmin helposti onnistunut 10 km juoksulenkki ei enää samanlailla onnistu. Klo 9 vastaanotolla neurologinen status on normaali. Asiallinen potilas, puhe on selkeää, mimiikka intakti. Silmien liikkeet laajat, ei kaksoiskuvia, ei nystagmusta, ei kaksoiskuvia. Lihasvoimat kertatestauksissa normaalit, toistotestauksissa ylä- ja alaraajoissa ei poikkeavaa väsymistä. Sensoriikka normaalia. Jännevenytysheijasteet ovat normaalit, Babinski -/-. 

- a. Työdiagnoosisi? (0,5p)
- b. Vastaanottoaikasi on jo täynnä. Tee lyhyt yhteenveto potilaalle tilanteesta ja jatkosuunnitelmasta (ml. työdiagnoosin mekanismi oireiden taustalla ja tarvittavat jatkotutkimukset pth/esh). Mitä sanot? (1,5p)
...
Myastenia gravis. Sille on tyypillistä diurnaalinen eli päivän mittaa paheneminen oireissa ja se yleensä juuri alkaa potilaan tapaan okulaarisesti (diplopia ja ptoosi) ja usein myös bulbaarisesti (puheen epäselvyys ja väsyminen). Potilas voi olla oireeton aamuisin. Sensoriikka ja refleksit ovat normaalit tyypilliseen tapaan. 
...
Sanon potilaalle, että kyseessä voi olla monia syitä ja sen takia otetaan laajalti labroja (esim. PVK, CRP, TSH, Na, K, Ca, Krea, gluk, D-25, ALAT). Taustalla saattaa kuitenkin myös olla myasthenia gravis -niminen hermo-lihasliitoksen autoimmuunisairaus, jonka takia tehdään myös lähete neurologian poliklinikalle jatkoselvittelyihin. Siellä sitten otetaan mahdollisesti myasthenia-ENMG ja ACHRAb-vasta-ainemittaus jatkoselvittelyinä. Jos myasthenia todetaan, niin thx-tt tymooman selvittelynä yleensä jatkossa. Mahdollisesti ainakin aikaisemmin on käytetty edrofoni eli tensilon-testiä, jossa oireet paranevat sen annostelun jälkeen, mutta käyttö on vähentynyt. 

Myasthenia gravis siis on autoimmuunisairaus, jossa on vasta-aineita hermo-lihasliitoksen synapsinjälkeisiä asetyylikoliinireseptoreita vastaan ja tämän takia lihas-hermoliitos väsyy toistossa. Tämä aiheuttaa oireiden pahenemisen päivän mittaan. Yleensä se alkaa juuri potilaan tavoin silmistä ja aiheuttaa siellä kaksoiskuvia ja riippuluomea. Se voi myös affisioida bulbaarisesti ja aiheuttaa puheen väsymistä, mikä aiheuttaa potilaan puheen epäselvyyden. Juoksulenkilläkin väsyy helpommin. Nyt ei statuksessa tule selkeää esiin, koska potilaan oirekuva ei ole vielä merkittävästi raajojen alueella (voitaisiin kylläkin tehdä testit, joissa potilas katsoo pitkään ylöspäin tai räpyttelee runsaasti ja näin indusoida okulaarisia oireita). 
  </div>
</div> 

### Aivovammat

- a. Luettele keskeiset esitiedot ja tutkimukset aivovammapotilaan päivystysarviossa. (2 p)
- b. Mihin luokituksiin aivovamman vaikeusaste luokitellaan akuuttivaiheessa ja mitkä keskeiset tekijät vaikuttavat vaikeusasteen arviointiin ja miten? (2 p)
- c. Mitkä ovat tavallisia lievän aivovamman oireita ja miten hoidat niitä? (2 p)
...
Aivovammapotilaalta tulee aina selvittää yleisesti mitä on tapahtunut. Miten satutti itsensä (vammaenergian ja mahdollisten liitännäisvammojen arvio)? Oliko päihtyneenä? Onko ennen ollut vammoja? Oliko mitään esioireita? Viitteitä kouristelusta? Mitä muistaa tapahtumasta? Mitä oireita vamman jälkeen? Mitä muita sairauksia potilaalla on? Mitä lääkityksiä, varsinkin antikoagulatiivisia tai antitromboottisia? Aikaisempi toimintataso?

Tulisi myös mahdollisuuksien mukaan saada silminnäkijältä kuvaus tilanteesta, koska potilaat eivät usein muista tilannetta tarkasti. 

Aivovammapotilailta tulee myös aina arvioida, onko kallon tai sen pohjan murtumia (pään palpaatio, brillen hematoomat, proc mastoideuksen mustelma, likvorvuoto, hemotympanum), muita vammoja, onko päihtymistila yllä vielä? Muita neurologisia oireita? 

Aivovammapotilailta tulisi aina vähintään selvittää GCS, PTA (post-traumaattinen amnesia) ja tajuttomuuden kesto ja kirjata nämä. Tietysti ensimmäisenä aina ABC-arvio. PTA voidaan arvioida monella tavalla (esim. vaikka pyytää potilasta kuvailemaan tapahtumia kronologisessa järjestyksessä vamman jälkeen ja sitten vielä pyytää häntä muistamaan kolme sanaa; myöhemmin tulee testaamaan saman taas ja pyytää potilasta toistamaan annetut sanat), mutta se määritellään siten, että se on aika vammasta siihen saakka, kunnes potilas pystyy taas muodostamaan yhtenäisiä muistikuvia. Tajuttomuuden kestossa silminnäkijäarvio on tärkeä, koska tajuton potilas ei helposti tiedä kuinka kauan itse oli tajuttomana, varsinkaan jos vammaan liittyy retrogradista amnesiaa. 

Usein potilaalta katotaan peruslabroja (pvk, na, k, krea, crp, gluk) ja tarvittaessa sitten muutakin (Esim. jos merkittävää infektio-oireilua ja epäilee CNS-infektiota niin likvor). EKG ja telemetriaseuranta yleensä myös. Pään TT:tä ei aina rutiinisti tarvita, jos ei ole siihen indikaatioita (voi seurata Käypä Hoidon aivovamma-ohjeistuksen algoritmia sen suhteen).
...
Aivovamman vaikeusaste voidaan määrittää kliinisesti GCS:n, PTA:n ja tajuttomuuden keston perusteella.

Lievässä aivovammassa GCS on 13-15 puoli tuntia vammasta ja koko seurannan ajan, PTA on korkeintaan 24h ja tajuttomuus on korkeintaan 30 minuuttia. Jos kuvantaminen on tehty, niin pienet löydökset ovat mahdollisia.

Keskivaikeassa aivovammassa GCS on 9-12 puoli tuntia vammasta, PTA on korkeintaan viikon ja tajuttomuus on korkeintaan 24h.

Vaikeassa aivovammassa GCS on 8 tai alle puoli tuntia vammasta, PTA on yli viikon ja tajuttomuus on yli 24 tuntia. 
...
Lievän aivovamman jälkeen voi olla oireita pitkäänkin, usein kuitenkin vain korkeintaan muutamia viikkoja, mutta joillain jopa monia kuukausia. Yleisimpiä oireita varsinkin akuuttivaiheessa ovat väsymys ja runsas unen määrä, päänsärky, epäspesifit kognitiiviset oireet (vaikea keskittyä yms) ja ärtyneisyys.

Näiden hoidossa yleensä tärkeää on vain ohjeistaa potilasta palauttamaan toimintatasoaan hitaasti normaalille tasolleen ja levätä tarpeen mukaan runsaasti. Liiallista rasitusta kannattaa välttää. Omasta nesteytyksestä ja riittävästä ravinnosta kannattaa huolehtia. Päänsärkyyn parasetamolia ensisijaisesti.

Usein työkuvasta riippuen kirjoitetaan muutama päivä - kaksi viikkoa sairaslomaa, mutta jos oireita on vielä 2 viikon kohdalla (eikä mitään uutta punaista lippua), niin voidaan kyllä jatkaa sairaslomaa 1kk asti, jonka jälkeen oireiden pitkittyessä potilas kyllä kuuluu neurologian aivovammapoliklinikan arvioon. Masennus voi olla mahdollinen jälkioire, tämän hoito mahdollisesti jopa lääkityksellinen oreiden pitkittyessä. 

Ajokieltoa kannattaa antaa lievissä vammoissa R1 1kk ajan. 

  </div>
</div> 

### Nopeasti pahentunut selkäkipu ja kävelyvaikeus

62-vuotias työelämässä oleva nainen saapuu terveyskeskuksen vastaanotolle. Hänellä on anamneesissa lakunaarinen aivoinfarkti vasemmalla tyvitumakealueella, josta ei jäännösoireita, hyperkolesterolemia, verenpainetauti ja sepelvaltimotauti. Lääkitys atorvastatiini 20mg x1, klopidogreeli 75mg x1, bisoprololi 5mg x1 ja ramipriili 5mg x1. Nyt hakeutunut vastaanotolle kävelyvaikeuden vuoksi. Noin 2 viikkoa sitten alkanut pistely molemmissa jalkaterissä sekä kipu alaselässä, sittemmin myös käsissä. Nyt viimeisen parin päivän aikana kävely muuttunut vaikeaksi. Statuksessa toteat aivohermostatuksen normaaliksi, yläraajoissa voimat normaalirajoissa. Alaraajoissa heikkoutta: kyykystä ylös noustessa joutuu ottamaan tuolista tukea, kanta- ja päkiäkävely ei onnistu. Ei selviä sensorisia puutoksia. Jännevenytysheijasteet eivät tule esiin, babinskit fleksiot. Periferia lämmin ja pulssit palpoituvat normaalisti. 

- a. Työdiagnoosi ja jatkosuunnitelma? Vastaa lyhyesti ja perustele. (2 p)
- b. Kirjoita lähete ensisijaiseksi arvioimaasi RAD/KNF/KLF-tutkimukseen. (2 p)
- c. Minkälaisia tuloksia odotat saavasi tästä tutkimuksesta? (2 p) 
...
Nopeasti alkanut ja nousevasti etenevä sensorimotorinen oireisto herättää epäilyn Guillain-Barrén oireyhtymästä (GBS). Sille on tyypillistä nopeasti alkava oireisto, johon kuuluu sensoristen oireiden (kuten juuri pistely) lisäksi pääoireena yleensä alaraajoista nouseva lihasheikkous. Usein mukana on myös selkäkipua. Jännevenytysheijasteet myös yleensä nousevasti muuttuvat negatiivisiksi, tyypillisesti akillesheijasteista alkaen. 

Koska kyseessä on immuunivälitteinen polyradikuliitti (taustalla yleensä hengitysteiden ja GI-kanavan infektio pari viikkoa edeltävästi; yleisin on C. jejuni, muita mm. EBV, influenssa, CMV, mykoplasma), niin on ääreishermojen affisioitumista ja babinskit ovat fleksiot eli negatiiviset. Pulssistatus on normaali, koska kyseessä on ääreishermoston ongelma. 

GBS-potilaat tulee lähettää päivystykseen, koska se voi nopeasti edetä hengityslihasten heikkouteen ja siten muodostua hengenvaaralliseksi. Päivystyksessä sitten otetaan yleensä peruslabroja (mahdollisesti myös etiologisia selvittelyjä), likvor (proteiinit merkittävästi koholla ja vain lievä valkosolujen kohoaminen (<50) eli albuminosytologinen dissosiaatio), ENMG ja usein myös MRI poissulkemaan muita aiheuttajia. Hengitystoimintaa seurataan vähintään parin tunnin välein PEF-mittauksilla. 

Hoidossa yleensä IVIG tai vaikeissa tapauksissa plasmafereesi. 
  </div>
</div> 
...
62-vuotias työelämässä oleva nainen, anamneesissa lakunaarinen aivoinfarkti vasemmalla tyvitumakealueella, josta ei jäännösoireita, hyperkolesterolemia, verenpainetauti ja sepelvaltimotauti. Lääkitys atorvastatiini 20mg x1, klopidogreeli 75mg x1, bisoprololi 5mg x1 ja ramipriili 5mg x1.

Nyt hakeutunut vastaanotolle kävelyvaikeuden vuoksi. Noin 2 viikkoa sitten alkanut pistely molemmissa jalkaterissä sekä kipu alaselässä, sittemmin myös käsissä. Nyt viimeisen parin päivän aikana kävely muuttunut vaikeaksi.

Statuksessa aivohermostatus normaali, yläraajoissa voimat normaalirajoissa. Alaraajoissa heikkoutta: kyykystä ylös noustessa joutuu ottamaan tuolista tukea, kanta- ja päkiäkävely ei onnistu. Ei selviä sensorisia puutoksia. Jännevenytysheijasteet eivät tule esiin, babinskit fleksiot. Periferia lämmin ja pulssit palpoituvat normaalisti. 

p.k. ENMG. Onko GBS viittaavaa? 
...
ENMG:ssä jo 1. vrk kohdalla motoriset johtonopeudet hidastuvat, vasteet dispersoituvat/osin blokeerautuvat, F/H-latenssit pitenevät, ilmaantuu A-aaltoja, myös sensoriset johtonopeudet hidastuvat, vaikkei olisi kliinisesti tuntopuutoksia, neula-EMG:ssä yksikkökato ensimmäiset 14 vrk tyypillisessä GBS:ssa (demyelinoiva muoto)

<li>Koska immunologisvälitteinen vaurio ei kohdistu hermosäikeisiin täysin tasaisesti, osa säikeistä johtaa signaalia nopeammin ja osa hitaammin. Tämä aiheuttaa vasteen dispersion eli hajoamisen (ENMG-näytöllä signaali venyy ja madaltu). Jos myeliinituho on jossain kohtaa täydellinen, signaali pysähtyy kokonaan, mikä näkyy johtoratablokkina.</li>
...
<li>H-vasteet taas ovat ENMG:n vaste jänneheijasteelle; senkin latenssi yleensä pitenee. GBS iskee tyypillisesti aivan ensimmäisenä hermojuriin (lähellä selkäydintä), missä veri-hermo-este on heikoimmillaan. Koska F- ja H-vasteet joutuvat kulkemaan näiden tulehtuneiden hermojuurten läpi, niiden latenssi eli viive pitenee usein jo aivan sairauden ensimmäisinä päivinä, ennen kuin raajojen ääreisosissa havaitaan suuria muutoksia.</li>
<li>A-aallot ovat ylimääräisiä, vakioisen muotoisia sähköisiä vasteita, joita nähdään motorisen johtonopeusmittauksen yhteydessä ennen F-aaltoa tai sen jälkeen. Niiden ilmaantuminen kertoo siitä, että hermossa on meneillään demyelinisaatio tai aksonivaurio, joka saa hermoimpulssin "heijastumaan" tai hyppäämään patologisesti säikeestä toiseen matkalla selkäytimeen.</li>
<li>ENMG havaitsee tuntosäikeiden myeliinivaurion (ja siten NCV:n hidastumisen) GBS:ssä jo varhaisessa vaiheessa. Tämä auttaa erottamaan GBS:n puhtaasti motorisista sairauksista (kuten ALS tai tietyt myosiitit)</li>
<li>Neula-EMG:ssä kun potilasta pyydetään jännittämään lihasta täysillä, aivoista tuleva käsky tyssää hermossa oleviin johtoratasalpauksiin (blokkeihin). Lihakseen asti pääsee vain harvoja sähköisiä impulsseja. Näytöllä näkyy tällöin vain harvoja, yksittäisiä motorisia yksiköitä supistumassa -> yksikkökato. Ensimmäisen kahden viikon aikana lihaksessa näkyy vain tämä toiminnallinen kato ja vasta noin 2–3 viikon kuluttua alkaa näkyä varsinaisia lihaksen lepotilan poikkeavuuksia (kuten fibrillaatioita), jos hermovaurio etenee demyelinisaatiosta jo aksonikadon tasolle. </li>
  </div>
</div> 
...
Ummetus, REM-unen aikaiset käytöshäiriöt ja hajuaistin heikkeneminen ovat yleisiä prodromaalioireita Parkinsonin taudissa
...
Ennakko-oireina on voinut jopa 20 vuotta ennen motorisia oireita ilmentyä mm. ummetusta, hajuaistin heikentymistä, REM-unen käyttäytymishäiriötä, päiväaikaista väsymystä ja lihaskipuja.
...
Parkinsonin taudin näköharhojen hoidossa rauhoitetaan tilanne ensin neuroleptillä
...
Parkinsonlääkkeiden käyttöön liittyy usein psykoosioireita (kuten näköhäiriöitä), mutta näiden hoitoon ei saa koskaan antaa perinteisiä neuroleptejä.
...
Syväaivostimulaatiohoitoa voidaan käyttää vaikeaoireisen Parkinsonin taudin ja essentiaalin vapinan hoidossa
...
Tarvittaessa lääkehoitojen ollessa riittämättömiä voidaan tukeutua neuromodulaatiohoitoihin, kuten juuri DBS tai neuro-HIFU. 
...
Levottomien jalkojen diagnoosi tehdään yöpolygrafian perusteella
...
RLS:n diagnostiikka perustuu ensisijaisesti anamneesiin (Diagnostinen kysymys: "Kun illalla yrität rentoutua tai nukahtaa illalla tai yöllä, onko sinulla koskaan epämiellyttäviä, levottomia tuntemuksia jaloissasi, jotka helpottuvat, kun liikutat jalkojasi tai kävelet?”) ja sekundaaristen syiden poissulkuun (esim. ferrit, polyneuropatia, tsh). Kliinisessä tutkimuksessa neurologinen status on normaali. Jos taas olisi esim. polyneuropatiaa oireiden taustalla -> värinätunto poikkeava, akillesrefleksit uupuvat, sukkamaiset tuntemukset.

Yöpolygrafiaa saatetaan tarvita samanaikaisen uniapnean poissulkemiseen tai jaksottaisen raajaliikehäiriön (PLMD) todentamiseen tarvittaessa. 
  </div>
</div> 